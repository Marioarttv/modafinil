    ...
    func scheduleWake(
        ...
        remoteProxy { proxyResult in
            ...
            case .failure(let error):
                finish(.failure(error))
            case .success(let proxy):
                proxy.scheduleWake(at: date) { success, message in
                    if success {
                        finish(.success(()))
                    } else {
                        finish(.failure(HelperError(message ?? "The helper could not schedule the wake event.")))
                    }
                }
            }
        }
    ...
    func cancelScheduledWake(
        ...
        remoteProxy { proxyResult in
            ...
            case .failure(let error):
                finish(.failure(error))
            case .success(let proxy):
                proxy.cancelScheduledWake(at: date) { success, message in
                    if success {
                        finish(.success(()))
                    } else {
                        finish(.failure(HelperError(message ?? "The helper could not cancel the wake event.")))
                    }
                }
            }
        }
